...
Executor path invariant: Runtime-generated `workingDirectory` and `targetPaths` identify the Subtask-owned Project worktree, while each Executor also needs a private runtime home for provider configuration and tools. Recoverable Pi session JSONL is a third, persistent path contract under the Runtime data root; it is pinned before launch and is never nested in or deleted with the attempt-private home. Planner never chooses host paths, session locators or branch names. Every registration binding declares an absolute binary path and source runtime home; the selected driver materializes an attempt-private home before process launch and never relies on implicit `HOME` discovery. Pi sets `HOME`, `PI_CODING_AGENT_DIR`, and `PI_CODING_AGENT_SESSION_DIR`, pre-creates the persistent session directory, and confirms the native header before the locator becomes continuation evidence. Codex sets an isolated `CODEX_HOME` containing the rewritten provider config. Hermes materializes its private config, and generic `cli-session` receives an empty isolated home plus only declared environment-file and inherited-variable sources. The child process `cwd` remains the Runtime-assigned worktree. Missing worktree, binary, source home, materialized home or an authorized continuation locator fails with a path-specific diagnostic.

The unreleased product uses fresh-only SQLite schema version 37. Fresh databases start at v37; every v36 or older pre-release database is rejected with its exact path and there is no migration, automatic deletion, or dual-read path. Schema v37 retains the Project, publication, Executor Registry, process-runtime, purge, immutable `kernel_decisions`, resource/workspace/permission/sandbox, dispatch/publication audit, cancellation cleanup, lease revocation, generation replan, deferred availability, bounded Executor recovery and `full | partial_accepted` revision facts. The existing `executor_attempt_runtime` row keeps only current Pi session ownership, binding/config identity, workspace identity, chain locator/native ID, confirmation/poison state and active-writer ownership; recovery-packet workspace baseline/delta/progress duplication is gone. Generic `kernel_events` and `kernel_decision_applications` are not created. `kernel_effect_outbox` remains the specialized task-completion delivery/provider-receipt boundary, and workspace checkpoints remain where permission suspension or non-Git recovery still needs them. `task_events` retain their domain history until an explicitly authorized terminal Task purge. `awaiting_decision` attempts recover from immutable terminal receipts, due retries from the current Kernel Decision, and startup reads current Task/Subtask/dispatch/session/worktree/claim/lease/permission/publication facts before submitting normalized recovery observations to Kernel. Executor Skill progress remains bounded attempt-lifetime verifier evidence, while only terminal Skill outcomes are persisted and atomically update effect summaries.
...
When touching dispatch, update focused behavior tests around `ControlKernel`, `KernelWorkflowRunner`, the immutable decision ledger, current-fact startup recovery, work-graph runtime, work-unit claims and attempt landing. Attempt terminal regressions remain anchored in `tests/execution/subtask-attempt-runner.test.ts` and `tests/session/planning-agent-session-routing.test.ts`.
