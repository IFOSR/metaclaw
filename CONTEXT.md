...
The unreleased product uses fresh-only SQLite schema version 36. Fresh databases start at v36; every v35 or older pre-release database is rejected with its exact path and there is no migration, automatic deletion, or dual-read path. Schema v36 retains the v35 Project, publication, Executor Registry, process-runtime, purge and durable workflow baseline while extending the existing `executor_attempt_runtime` record with the minimum current Pi session facts: ownership, binding/config identity, workspace identity, chain locator/native ID, confirmation/poison state and active-writer ownership. It does not add a general journal and does not remove or stop the v35 workflow/event/application/outbox/checkpoint writes; those are explicitly deferred beyond Session-first Phase 1. Persisted Planner proposal turns/submissions and their accepted-turn lock, the Kernel v5 decision/workflow ledger, resource/workspace/permission/sandbox facts, durable dispatch/publication audit, cancellation cleanup, lease revocation, generation replan requests, deferred availability proposals, bounded Executor recovery checks and `full | partial_accepted` revision completion remain intact. `kernel_events` and `task_events` keep their complete durable history until an explicitly authorized terminal Task purge. Executor Skill progress remains bounded attempt-lifetime verifier evidence, while only `skill_completed`, `skill_failed`, `skill_skipped` and `skill_suggested_patch` events are persisted; the same transaction updates `skill_effect_summaries` through its owning repository. Learning candidates reference the source Skill usage event directly; reflection, guidance and executor-route event tables are not part of the schema. `awaiting_decision` and `awaiting_integration` remain Subtask-only states; startup recovery reconciles applications, child items, cancellation cleanup, worktree process records, session-writer ownership, leases, publication state and valid unresolved publication-review presentation before accepting input.
