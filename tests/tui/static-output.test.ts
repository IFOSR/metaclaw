...
const staticCapture = vi.hoisted(() => ({
  calls: [] as unknown[][],
}));
...
vi.mock('ink', async () => {
  ...
  return {
    ...
    Static: ({ items, children }: { items: unknown[]; children: (item: unknown, index: number) => React.ReactNode }) => {
      staticCapture.calls.push([...items]);
...
describe('App static transcript rendering', () => {
  it('renders transcript output through Ink Static so typing does not remount history lines', async () => {
