...
> fresh-only SQLite schema v37 with no pre-release upgrade path, one explicit
...
- Enforces one active top-level task through the serial `KernelWorkflowRunner`, while Phase 6 authorizes deterministic batches and supervises up to four isolated child attempts concurrently.
- Keeps Planning and Runtime authorization in one append-only `kernel_decisions` ledger while current domain rows, native sessions/worktrees and specialized receipts own recoverable execution.
...
- Validates every Subtask through Completion Protocol v4, computes the authoritative workspace delta independently for Git verification, persists clean results and immutable direct-edge handoffs, and allows exactly one Kernel-authorized same-AgentClass full retry after the first contract failure.
...
flowchart LR
  ...
  Event --> Loop[KernelWorkflowRunner v5<br/>snapshot, decide, apply]
  ...
  Session <--> Store[(Local SQLite schema 37<br/>projects, tasks, approvals,<br/>work units, decisions, memory)]
...
flowchart LR
  ...
  Decision --> Apply[KernelWorkflowRunner Runtime apply]
...
Every natural-language proposal and deterministic execution entrypoint enters the same control chain: `event → bounded snapshot → ControlKernel.decide → kernel_decisions → Runtime apply → normalized event`. `KernelWorkflowRunner` remains serial, but applying `dispatch_batch` only persists `kernel_dispatch_items`; an Execution-owned supervisor launches them asynchronously and submits each outcome independently. A sibling failure never cancels the rest of the batch.
...
`KernelWorkflowRunner` serializes one event at a time, finds or issues its immutable `kernel_decisions` authorization, invokes the Runtime handler, and immediately submits stable observations through the same seam. Deterministic duplicate events reuse the existing Decision. Startup does not replay an inbox or reconcile generic applications: it reads current Task/Subtask, dispatch, immutable receipt, native session/worktree, claim/lease, permission/publication and outstanding Decision facts, then gives normalized recovery observations to Kernel. Planner runs and bounded redacted tool summaries remain audited separately. `WorkGraphRuntimeService` derives graph facts without selecting strategy. `KernelExecutionRuntime` builds snapshots and applies decisions; `AttemptSupervisor` owns child launch; `SubtaskAttemptRunner` atomically lands receipts and current attempt/publication facts; `WorkspacePublicationWorker` applies user-approved Project-main promotions and atomically publishes completion facts.
...
`Subtask` records only after an `authorize_task_plan` Decision is applied. The
unreleased product uses fresh-only SQLite schema v37; every v36 or older
pre-release schema is rejected with its exact path, with no migration,
automatic deletion or dual-read path. Schema 37 retains Project, publication,
Executor Registry, process-runtime, purge, immutable Kernel decisions, graph revisions,
resource/workspace/permission/sandbox records, dispatch/publication/immutable merge audit,
cancellation cleanup, lease revocation, generation replan, deferred availability,
bounded recovery checks and partial completion facts. The existing attempt-runtime
record keeps only current Pi session ownership, binding/config, workspace,
locator/native ID, integrity and active-writer facts. Generic Kernel event/application
replay and recovery-packet workspace baseline/delta/progress fields are absent.
The specialized completion-delivery outbox and Task event history remain.
...
the retained bounded recovery packet/fallback, or blocking. Startup reconstructs only
current recovery facts; generic workflow event/application replay is absent, while specialized
delivery, publication, permission and still-consumed workspace-checkpoint records remain.
