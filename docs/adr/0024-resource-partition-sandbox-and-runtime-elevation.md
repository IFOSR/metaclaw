...
For the local interactive Pi surface, the exact request is projected only while its current escalation Decision and unresolved request/publication facts remain valid for 24 hours. Pi uses a transient native button Selector and submits only request ID plus approve/deny with `source: button`; it never stores the request or resolution in conversation context. The shared `/permission` command and interactive Planner authorization proposal are unavailable. A resolution Decision plus its current permission facts is authoritative. Same-button replay is idempotent; opposite, cross-session, resolved, stale, and expired submissions conflict. Feishu, RPC, and Session Planner retain the exact natural-language path described above.

### Persistence and recovery

SQLite schema 37 separates resource leases/waits, workspace/checkpoints/content references, permission requests/grants/user authorizations, attempt process lifecycle and the minimum persistent Executor-session identity on the existing attempt-runtime record. Recoverable Pi session JSONL lives under the Runtime data root; attempt-private Home/cache/log cleanup cannot remove it. A partial unique active-writer gate serializes one session locator without reducing concurrency between different Subtasks. The old unused worktree lease shape remains legacy audit. Resource and WorkUnit leases are attempt-bound, heartbeat-driven and idempotent. Startup reads active attempt records, makes a best-effort termination of each recorded PID, seals terminal dispatch/receipt facts, then releases reconciled session/claim/lease ownership and passes normalized current recovery facts to Kernel. Generic workflow event/application replay and recovery-packet workspace baseline/delta/progress persistence are absent; permission suspension and non-Git restoration may still use their specialized workspace checkpoints. Pre-release schema 37 is fresh-only; v36 and older databases are rejected rather than migrated or dual-read.
