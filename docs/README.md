...
- [ADR-0023: Durable Kernel Workflow, Recovery And Availability](adr/0023-durable-kernel-workflow-recovery-and-availability.md): Kernel decision authority, current-fact startup recovery, structured failure, retry/fallback, availability, continuation and specialized external-effect delivery.
...
- [Executor Session-first 持久化与恢复简化](plans/2026-08-14-executor-session-first-persistence-recovery-simplification.md)：两阶段均已完成；Pi Executor session 位于持久数据根目录，启动从当前 Task/attempt/session/worktree/receipt/permission/publication 事实恢复，通用 Kernel event/application replay 已删除。该计划取代 2026-08-11 与 2026-08-13 的旧提案。
