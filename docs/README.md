...
- [Executor Session-first 持久化与恢复简化](plans/2026-08-14-executor-session-first-persistence-recovery-simplification.md)：第一阶段把 Pi Executor session 移入持久数据根目录，增加最小 locator/runtime/workspace/current-writer 事实并把 `resume | fresh | blocked` 交给现有 Kernel；第二阶段的删表、停写与通用 replay 缩减尚未开始。该计划取代 2026-08-11 与 2026-08-13 的旧提案。
